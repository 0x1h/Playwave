...
const Form: FC<{ appearComponent: () => void }> = ({ appearComponent }) => {
  ...
  const handleSubmit = (e: React.ChangeEvent<HTMLFormElement>): void => {
    ...
    if (
      !formHandle.name.trim() ||
      !formHandle.surname.trim() ||
      !formHandle.nickname.trim()
    ) {
      setIsError(true)
  ...
  const setImage = (dataURL: string): void => {
    setFormHandle({...formHandle, imgSrc: dataURL})
  }
        ...
        <form onSubmit={handleSubmit} className="form-box">
          <div className="base-inputs">
            ...
            >
              {formHandle.imgSrc ? (
                <img
                  src={formHandle.imgSrc}
                  style={{ width: "100%", position: "absolute" }}
                  alt=""
                />
              ) : (
                <DisplayEmptyImage uploadImage={setImage}/>
              )}
            </div>
